...
x_mesh, y_mesh, z_mesh = torch.meshgrid(torch.linspace(0, Lx, Nx), torch.linspace(0 , Ly, Ny), torch.linspace(0, Lz, Nz),indexing='ij')
X = torch.stack([x_mesh.flatten(), y_mesh.flatten(), z_mesh.flatten()], dim=1)
...
N = X.shape[0]
F = torch.zeros(N, 3,3, dtype=torch.float32) #Deformation gradients
...
#for i in range(len(alpha)):
    #for j in range(len(mu)):
        #psi = (mu[j]/alpha[i])*(λ1**alpha[i]+ λ2**alpha[i]-3)
#print(psi)
